...
class BuildWaveFunctionTests(unittest.TestCase):
    def test_BuildWaveFunctionSuperposition(self):
        ...
        TestHelpers.compareMatricies( self,rtnPsi.data, np.array([1/np.sqrt(2), 1/np.sqrt(2)]))
    ...
    def test_BuildWaveFunctionBellState(self):
        tokens = ["Cnot", "(", "H", "I", ")", "|00>"]
        rtnPsi = qh.buildWaveFunction(tokens)
        TestHelpers.compareMatricies( self,rtnPsi.data, np.array([1/np.sqrt(2),0,0,1/np.sqrt(2)]))
    ...
    def test_buildWaveFunctionExponential(self):
        tokens = ["(", "Exp", "2", ")", "|0>"]
        rtnPsi = qh.buildWaveFunction(tokens)
        TestHelpers.compareMatricies( self,rtnPsi.data, np.e**2 * qh.buildKet("|0>").data)
